...
;
        ...
        private bool voting_open(string type)
        {
            MySqlConnection con = new MySqlConnection("server=localhost;port=3308;username=root;password=;database=e_ballot");
            string query = "select closed from voting_time where seat_type = '" + type + "';";
            con.Open();
        ...
        private void Load_grid_pm()
        {
            string str = "server=localhost;port=3308;username=root;password=;database=e_ballot";
            MySqlConnection con = new MySqlConnection(str);
            con.Open();
            String query = "select ni.NAME , p.PARTY_NAME , ni.CNIC from party p, nadra_info ni where p.party_leader = ni.CNIC;";

            MySqlCommand cmd = new MySqlCommand(query, con);
            MySqlDataReader reader = cmd.ExecuteReader();
            pm_dataGridView.Rows.Clear();

            while (reader.Read())
            {
                try
                {
                    pm_dataGridView.Rows.Add(reader.GetString(0), reader.GetString(1), reader.GetString(2));
                }
                catch
                {
                    con.Close();
        ...
        private void check_voted_pm()
        {
            string str = "server=localhost;port=3308;username=root;password=;database=e_ballot";
            MySqlConnection con = new MySqlConnection(str);
            String query = "select vs.VOTER_CNIC from voting_pm vs where vs.VOTER_CNIC = '" + cnic + "';";
            MySqlCommand cmd = new MySqlCommand(query, con);
            con.Open();
            MySqlDataReader reader = cmd.ExecuteReader();
            try
            {
                ...
                if (cnic == reader.GetString(0))
                {
                    vote_button4.Enabled = false;
                    vote_label4.Text = "Already voted!";
                }
            }
            catch
            {
        ...
        private void E_Ballot_1_Load(object sender, EventArgs e)
        {
            ...
            {
                if (voting_closed("Open Seat"))
                {
                    check_if_won_open_seat();
                    find_prvince();

                    if (voting_open("Reserved Seat") && tabPage2.Enabled == true)
                    {
                        Load_grids_resereved();
                        check_voted();               
                    }
                    if (voting_open("Senate Seat") && tabPage3.Enabled == true)
                    {
                        Load_grids_senate();
                        check_voted_senate();
                    }
                    if (voting_open("PM Seat") && tabPage4.Enabled == true)
                    {
                        Load_grid_pm();
                        check_voted_pm();
                    }
            ...
            {
                if (voting_closed("Reserved Seat"))
                {
                    find_province_reserved();
                    check_if_won_reserved_Seat();
                    
                    if (voting_open("Senate Seat") && reserved_won_check == true)
                    {
                        tabPage3.Enabled = true;
                        Load_grids_senate();
                        check_voted_senate();
                    }
                    if (voting_open("PM Seat") && reserved_won_check == true && status_label.Text == "MNA")
                    {
                        tabPage4.Enabled = true;
                        Load_grid_pm();
                        check_voted_pm();
        ...
        private void vote_button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Plesae select a candidate!");
            }
        }
        ...
        private void vote_button4_Click(object sender, EventArgs e)
        {
            if (pm_cnic != "NULL")
            {
                string str = "server=localhost;port=3308;username=root;password=;database=e_ballot";
                MySqlConnection con = new MySqlConnection(str);
                String query = "insert into voting_pm values('" + cnic + "','" + pm_cnic + "');";
                MySqlCommand cmd = new MySqlCommand(query, con);
                con.Open();
                cmd.ExecuteReader();
                con.Close();
        ...
        {
            int rowIndex = e.RowIndex;
            DataGridViewRow row = pm_dataGridView.Rows[rowIndex];
            pm_cnic = Convert.ToString(row.Cells[2].Value);
            MessageBox.Show("Candidate Selected : " + Convert.ToString(row.Cells[0].Value));
        }
    }
}
