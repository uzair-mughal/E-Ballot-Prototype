...
namespace Candidate_Panel
{
    public partial class Apply_open_seat : Form
    {
        ...
        private bool seat_taken()
        {
            string desig = desg_comboBox.Text[1].ToString() + desg_comboBox.Text[2].ToString();
            string str = "server=localhost;port=3308;username=root;password=;database=e_ballot";
            MySqlConnection con = new MySqlConnection(str);
            con.Open();
            String query = "select PARTY_ID from party where PARTY_NAME = '" + party_label.Text + "'";
            
            MySqlCommand cmd = new MySqlCommand(query, con);
            MySqlDataReader reader = cmd.ExecuteReader();
            reader.Read();
            int party_id = reader.GetInt32(0);

            con.Close();

            query = "select ac.CNIC from approved_candidates ac, party p where p.PARTY_ID = ac.PARTY_ID and ac." + desig + "_REGION_ID = " + regionid + " and p.PARTY_ID =" + party_id + ";";
            String query1 = "select cr.CNIC from candidancy_request cr, party p where p.PARTY_ID = cr.PARTY_ID and cr." + desig + "_REGION_ID = " + regionid + " and p.PARTY_ID =" + party_id + ";";
